...
Индексация новых рекламных лендингов в первом релизе была выключена отдельно от
их публичной доступности. После редакционного утверждения, добавления обеих
страниц в sitemap и подтверждения URL-prefix property Яндекса этот временный
gate должен быть переключён на `ROSPARK_LANDING_INDEXABLE=true` отдельным
