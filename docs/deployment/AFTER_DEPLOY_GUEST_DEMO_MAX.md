...
> Актуализация 2026-07-23. Этот runbook был подготовлен как безопасный сценарий
> первого выпуска. Фактическое текущее состояние production зафиксировано в
> `docs/production/PRODUCTION_STATE_2026_07_23.md`: MAX уже проверен реальной
> demo-доставкой, а SQLite пока находится внутри Git checkout. Требования ниже
> про `DEMO_MAX_ENABLED=false` и базу вне checkout описывают исходный baseline
> или целевое состояние, но не должны автоматически применяться к работающему
> production. Любое переключение выполняется отдельным согласованным выпуском.
...
Для безопасного baseline первого выпуска предполагались server-side значения:
...
- целевое значение `DEMO_REQUESTS_DB_PATH` должно указывать на постоянный каталог
  вне Git checkout; текущий production-путь переносится только отдельным
  согласованным окном;
- `DEMO_MAX_ENABLED=false` обязательно только при выпуске по исходному baseline;
- если в утверждённом production-релизе MAX уже включён, его состояние сначала
  фиксируют read-only и не переключают в рамках документарной сверки;
- GREEN-API credentials не добавляются и не меняются без отдельного разрешения;
...
5. фиксирует фактическое значение `DEMO_MAX_ENABLED`, не меняя его без
...
- [ ] Фактическое значение `DEMO_MAX_ENABLED` зафиксировано; изменение отдельно
      согласовано.
...
- [ ] MAX не переключался без отдельного разрешения; при включённом MAX выполнен
      согласованный безопасный smoke.
