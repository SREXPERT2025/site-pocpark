...
План следующего согласованного выпуска: добавить события полной AI-воронки
`rospark_ai_chat_open` → `rospark_ai_first_message_sent` →
`rospark_ai_engaged_chat` → `rospark_ai_lead_handoff`. До публикации кода и
отдельной настройки интерфейса Метрики эти четыре цели не считаются созданными.
