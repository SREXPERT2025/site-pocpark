...
## Последующее gateway hardening

После выпуска сайта gateway Mac Studio обновлён независимо до release:

`80ffd254003eb45c0026db0598c83a0f6d5e830c`.

Подтверждено:

- prewarm responder context и быстрые детерминированные маршруты;
- исправленный сценарий собственного идентификатора и доступных способов
  оплаты;
- allowlist-каталог контекстных ссылок сайта;
- production LaunchAgent использует release `80ffd25`;
- SHA сайта на VPS при этом не менялся.

## Следующие задачи

1. Продолжить review реальных диалогов и дополнять FAQ/маршрутизацию на
   Mac Studio.
2. Измерять uncached latency и расширять быстрые маршруты без ухудшения Fact
   Gate и lead boundary.
3. Выполнить discovery интеграции с amoCRM; до неё локальный lead registry
   остаётся источником истины для сайта.
4. `LANDING-GROWTH-001` использовать как отдельный источник разговоров и
   лидов после визуальной приёмки.
5. `SECURITY-RELEASE-2` сохранить последним этапом согласно решению владельца.

Задача сохранения MAX provider receipt закрыта в production release
`47529cc`: новые outbox-записи сохраняют message ID, destination ID и время
