...
- мобильные устройства и пользователи с reduced motion получают статичный
  Hero без загрузки фонового MP4;
- desktop сохраняет согласованное видео и получает тот же poster до старта;
- изображения типов объектов переведены на responsive Next Image;
- robots.txt закрывает служебные API, админку, embed, quiz и закрытые demo,
  сохраняя публичный `/demo` и коммерческие страницы доступными для обхода;
- добавлены регрессионные тесты Hero media и технического SEO-контракта;
- на 390 px MP4 не загружается, на 1440 px видео воспроизводится; production-
  сборка всех 116 страниц и локальные cache/robots smoke прошли;
- production пока не изменялся: блок включается в накопительный релиз.

## 2026-08-06 — первый SEO/GEO-пакет по реальному поисковому спросу

- проверены 54 утверждённых поисковых запроса и их целевые страницы;
- широкий кластер автоматизации закреплён за главной, системный — за
  `/resheniya`, кластер оборудования — за `/oborudovanie`;
- на главной уточнены metadata и первый ответ, добавлены контекстные ссылки на
  два профильных хаба;
- `/resheniya` явно отвечает на запрос об автоматизированной парковочной
  системе и отдельно ведёт к сценарию «Парковка под ключ»;
- `/oborudovanie` объясняет состав оборудования для платной и закрытой
  парковки и ведёт к шлагбаумам, стойкам и терминалу оплаты;
- рекламные лендинги и узкие товарные страницы не переписаны, чтобы не
  создавать каннибализацию;
- добавлена регрессионная проверка кластеров и внутренних ссылок;
- production пока не изменялся.

## 2026-08-06 — локальный GEO/entity и mobile-home кандидат

- 12 статей получили подтверждённые даты публикации, автора
  `Команда РОСПАРК` и юридического издателя ООО «СР Эксперт»;
- Organization JSON-LD использует подтверждённый логотип `/logo.png`,
  неподтверждённые внешние профили не добавлялись;
- для 11 подтверждённых кейсов нормализованы название объекта, город, регион,
  тип и оборудование; CaseStudy JSON-LD использует только эти данные;
- исправлены выявленные расхождения AMAKS, «Горизонта», W-Plaza,
  «Элма-Курьяново» и «Мосфильма»;
- W-Plaza не выдан за введённый объект, а закрытая причина ожидания не
  опубликована;
- два входа в рекламные лендинги на главной адаптированы для мобильных экранов;
- на 320/375/390/1440 px нет горизонтального переполнения карточек;
- целевые тесты, TypeScript, ESLint и production-сборка 116 страниц под
  Node.js 22 прошли;
- production и внешние сервисы не изменялись.

## 2026-08-05 — подготовлен measurement и source gate следующего цикла

- создана единая матрица срезов GA4, Метрики, Google Search Console и Яндекс
  Вебмастера на 24 часа, 3–7 и 14 дней;
- зафиксирован последний доступный GSC-срез до 3 августа и отсутствие свежих
  GA4-событий без генерации искусственной заявки;
- подготовлен шаблон подтверждения фактов для первых трёх страниц
  `GEO-CONTENT-001`;
- персональные данные и закрытые документы в шаблон не переносятся;
- production и внешние кабинеты не изменялись.

## 2026-08-05 — production GEO dates и Hero performance (`1351b93`)

- опубликованы только подтверждённые редакционные даты; шаблонная дата
  `2025-12-21` удалена из 30 кейсов;
- sitemap содержит 80 URL и 11 подтверждённых `lastmod`;
- Hero-видео главной уменьшено примерно с 12 МБ до 1,76 МБ и получает
  immutable cache header;
- публичные страницы, schema, sitemap, видео и WebP прошли HTTPS acceptance;
- production SHA — `1351b9334ef9e5a6017b4852783dd70e40d94c71`, PM2 online;
- сохранены backup и hot rollback, лиды и сообщения в MAX не создавались;
- свежий GSC-срез заканчивается 3 августа, а GA4 пока показывает 0 недавних
  событий, поэтому следующий контентный пакет остаётся за evidence gate.

## 2026-08-05 — подготовлен кандидат оптимизации Hero-видео

- активное видео главной перекодировано без изменения разрешения 1920×1080 и
  частоты 24 fps: 12,0 МБ / 19 Мбит/с → 1,76 МБ / 2,8 Мбит/с;
- объективная близость кадров SSIM — 0,98, сравнительный кадр проверен визуально;
- новый ассет имеет версионное имя, оригинал сохранён для отката;
- для версионного видео подготовлен точечный immutable cache header и тест
  бюджета 2 МБ;
- production пока не изменялся.

## 2026-08-05 — подготовлен кандидат редакционных дат

- подтверждено по истории git, что одинаковый `lastModified: 2025-12-21` у
  30 кейсов был унаследован из шаблона и не является датой каждого объекта;
- шаблонная дата удалена из кейсов, filesystem mtime больше не используется как
  публичная редакционная дата;
- явные корректные даты нормализуются в ISO, отсутствующие и невалидные
  значения не публикуются в sitemap и JSON-LD;
- целевой тест, TypeScript, ESLint и production build 116 страниц прошли;
- production пока не изменялся.

## 2026-08-05 — подготовлен analytics и legacy-SEO кандидат

- GA4 подтвердил отсутствие трафика и недавних событий за 48 часов при
  правильном Measurement ID `G-3Z9KNN3MMK`;
- загрузчик приведён к официальному native `Arguments` envelope `gtag.js`,
  добавлена регрессионная проверка;
- Search Console показал 1 клик и 19 показов старого
  `/mobile/index.html`; для него подготовлен 308 на каноническую главную с
  сохранением query string;
- настройки внешних кабинетов и production не изменялись.
...
- 13 используемых на `/parkovka` PNG/JPG заменены ссылками на визуально
  проверенные WebP; исходные файлы сохранены;
- оптимизируемый объём уменьшен с 10,23 МБ до 0,93 МБ, общий raster payload
  лендинга — около 1,41 МБ;
- добавлена автоматическая защита от возврата тяжёлых raster assets;
- двум утверждённым лендингам добавлена безопасная Service JSON-LD, а видимому
  FAQ страницы «Парковка под ключ» — FAQ JSON-LD;
- тексты, дизайн, формы, AI-виджет и production не изменялись.

## 2026-08-05 — индексируемые лендинги `2f8b2b2`

- `/parkovka` и `/parkovka-pod-klyuch` опубликованы с `index, follow` и
  корректными canonical;
- актуальный sitemap повторно отправлен в Google Search Console и поставлен на
  переобход в Яндекс Вебмастере;
- production SHA `2f8b2b2b914f7c24c03e2693ccf3e036e1a68d44`, PM2 `online`;
- `LEADS_CREATED=0`, `MAX_MESSAGES_SENT=0`.
...
- добавлен HTML verification-файл для дополнительной URL-prefix property
  Яндекс Вебмастера;
- утверждённые лендинги `/parkovka` и `/parkovka-pod-klyuch` добавлены в
  sitemap;
- страницы оборудования перестают заявлять Product/Merchant rich results без
  подтверждённой публичной цены, отзывов и рейтинга;
- каталог сохраняет структурированный список страниц оборудования без
  фиктивных Offer;
- 23 URL «Обнаружена, не проиндексирована» не удаляются: все существуют и
  требуют переобхода и выборочного усиления содержания;
- проверены все 20 опубликованных кейсов: обязательные структурные элементы
  присутствуют, поэтому массовая перепись не выполнялась;
- из профильных страниц решений добавлены естественные ссылки на шесть
  подтверждённых кейсов торговых центров, бизнес-центров и складского
  комплекса;
- production, Nginx, поисковые настройки, лиды и MAX этим кандидатом не
  изменялись.

## 2026-08-04 — production SEO/GEO baseline `0c2b931`

- опубликованы разные meta description для шлагбаумов РОСПАРК 3 м и 4 м;
- документация дополнена фактическим первым срезом Google Search Console и
  Яндекс Вебмастера, точными группами 404 и обнаруженных без обхода URL;
- release выполнен fast-forward поверх AI context hotfix `5223dd8`;
- staging прошёл typecheck, lint, AI tests и production build 116 страниц;
- production service завершилась с `Result=success`, приложение осталось
  `online`;
- публичная проверка подтвердила новые description, canonical и доступность
  контрольных индексируемых страниц;
- лиды не создавались, сообщения в MAX не отправлялись;
- текущий production SHA:
  `0c2b9310016f3f044a2d544c6c4aac2dd3a236e2`.

## 2026-08-04 — production hotfix AI-контекста `5223dd8`

- исправлена отправка вопросов AI-консультанту с обычных страниц сайта:
  отсутствие контекста лендинга больше не превращается в пустой объект,
  отклоняемый сервером;
- строгая проверка разрешённых значений `/parkovka` и
  `/parkovka-pod-klyuch` сохранена;
- устранена календарная зависимость release-теста retention журнала;
- staging и production release прошли автоматические проверки, PM2 остался
  `online`;
- внешний browser smoke на главной странице получил полноценный ответ со
  ссылками вместо сообщения «Запрос отклонён»;
- заявка не создавалась, сообщения в MAX не отправлялись;
- текущий production SHA:
  `5223dd80f6db384ca52685088522c9aba4c3f86a`.

## 2026-08-04 — первый post-release SEO/GEO baseline

- read-only срез Search Console за доступный период подтвердил 11 кликов, 250
  показов, CTR 4,4% и среднюю позицию 18,1;
- Google показывает 57 проиндексированных и 55 исключённых URL; основные
  рабочие группы — 23 обнаруженных, но не проиндексированных URL, 5 ответов
  `404` и 24 canonical-варианта;
- sitemap обработан успешно 02.08.2026, обнаружено 78 страниц;
- полевых данных Core Web Vitals пока недостаточно для мобильных устройств и
  ПК;
- Яндекс Вебмастер не показывает критических ошибок, но подтвердил одинаковый
  description у шлагбаумов РОСПАРК 3 м и 4 м;
- для обеих страниц подготовлены разные описания в ветке
  `fix/site-20260804-seo-baseline-1`;
- из Search Console выгружены и классифицированы точные списки: три
  `/index.html` признаны алиасами главной, назначение `/to.html` и
  `/overviews.html` оставлено неподтверждённым, а все 23 обнаруженных без
  обхода URL подтверждены существующими страницами сайта;
- production, поисковые настройки, ручной переобход и внешние отправки не
  изменялись;
- отдельно подготовлен exact-кандидат Mac Studio gateway `279d919`: 46 gateway
  tests, 13 cascade tests, 5 review tests и 13 живых контрольных вопросов
  пройдены; переключение остаётся за отдельным разрешением.

## 2026-08-04 — production release `279d919` и новый growth cycle

- опубликованы две согласованные рекламные страницы: `/parkovka` и
  `/parkovka-pod-klyuch`;
- `/puzzle2` перенаправляется на `/parkovka-pod-klyuch`, архивные варианты
  сохранены в Git и закрыты в production ответом `404`;
- лендинги встроены в общую навигацию, футер, релевантные страницы, cookie
  consent, lead registry, GA4, Метрику и AI-консультант;
- опубликована site-wide AI-воронка и безопасная передача контекста лендинга;
- `/parkovka` без предварительного выбора карточки принимается AI API, а
  произвольный или межстраничный контекст отклоняется;
...
- две предварительные попытки переключения автоматически откатились из-за
  ошибок контрольного release smoke; данные, лиды и MAX не затронуты;
- финальный выпуск завершён с `Result=success`, production SHA
  `279d919820938e4ea87dcdd7a6138774df55f8c1`, PM2 `online`;
- внешний smoke подтвердил `200` новых страниц, redirect совместимости и `404`
  старых preview routes;
- Mac Studio production gateway read-only проверен: он продолжает работать из
  `80ffd25`; подготовленная в `279d919` новая логика качества ответов требует
  отдельного gateway release;
- roadmap переключён с разработки лендингов на измерительный SEO/GEO цикл:
  поисковые кабинеты, карта спроса, entity/schema, answer-first улучшения,
  максимум три подтверждённых материала и 30-дневная оценка конверсии.
...
- build ID: `BP6pCB_Trq6mXe3Xh75wF`, Node.js `22.23.1`, Next.js `14.2.35`;
- опубликованы GA4, подтверждения доставки MAX и три обновлённые обложки
  статей;
- lead registry штатно обновлён до migrations `1–4`, базы прошли
  `quick_check`;
- PM2 и три operational timers активны;
- outbox сохранил `sent=2`, новых сообщений MAX выпуск не отправлял;
- публичные страницы, AI status, `/admin/leads/login` и sitemap прошли
  production HTTP smoke;
- desktop/mobile browser QA подтвердил новые изображения, публичный
  AI-виджет, отсутствие overflow и отсутствие виджета/аналитики в `/admin`;
- GA4 Measurement ID `G-3Z9KNN3MMK` и Метрика `110980303` присутствуют в
  production build и загружаются на публичном host после согласия;
- Google Analytics stream `15338809658` создан; подтверждение первого события
  и Realtime ожидает обработки Google до 48 часов;
- первый cutover безопасно откатился из-за ещё не применённой migration `4`;
  повторный выпуск выполнил отдельный backup, миграцию и полную проверку.

## 2026-07-28 — Google Analytics 4
...
- Google Signals, рекламные user data, рекламное хранилище и персонализация
  рекламы выключены;
...
- пакет опубликован на VPS; Measurement ID добавлен в production environment,
  consent browser smoke выполнен, Realtime остаётся на контроль после
  обработки первого события Google.

## 2026-07-28 — обновление трёх обложек статей

- обложка статьи «Оплата парковки гостей за счёт арендатора» заменена
  утверждённым изображением с подтверждением оплаты на экране;
- обложка статьи «Гостевой доступ на парковку» заменена сценарием разрешённого
  въезда со смартфоном и шлагбаумом;
- обложка статьи «Отчётность владельца парковки» заменена сценарием
  управленческой аналитики парковки;
- исходники приведены к существующему контракту `WebP 1920 × 1080`, поэтому
  URL страниц, front matter, карточки статей и Open Graph менять не пришлось;
- пакет опубликован на VPS в release `47529cc`.

## 2026-07-28 — `LEAD-OPS`: подтверждение доставки MAX

- подготовлен локальный пакет миграции `4` для сохранения ответа провайдера:
  MAX Message ID, Chat ID и времени принятия;
- worker передаёт подтверждение MAX в transactional outbox одновременно с
  переводом записи в `sent`;
- в карточке лида добавлен блок «Доставка MAX» со статусом, временем и
  раскрываемыми техническими идентификаторами;
- старые отправленные записи продолжают отображаться как доставленные, но
  provider ID у них остаётся пустым до отдельного backfill;
- lead registry, lead admin, lead CLI, typecheck, lint и production build
  `107/107` прошли;
- реальные заявки и сообщения MAX при проверке не создавались;
- пакет опубликован на VPS; migration `4` применена, старые отправки сохранены,
  будущие MAX-отправки будут сохранять provider receipt.
...
- добавлены отдельные production API routes, fail-closed конфигурация и
  server-side origin validation;
- временный in-memory rate limit заменён SQLite rate limit;
- диалоги журналируются на VPS до семи дней, operational cleanup вынесен в
  systemd timer;
- реальные заявки из виджета регистрируются в существующем lead registry,
  назначаются Сергею и ставятся в существующий MAX outbox;
- имя и телефон остаются только в lead registry и не передаются AI-модели;
- Андрей и Сергей видят все диалоги и связанные заявки, безвозвратное удаление
  доступно только Андрею;
- добавлен production fallback, сохраняющий возможность оставить заявку при
  недоступности Mac Studio;
- production-профиль gateway отделён от preview-профиля; процессы используют
  loopback-порты `8788` и `8787`;
...
- по обратной связи 2026-07-28 малозаметные точки ожидания заменены на
  заметный статус `Готовлю ответ` с последовательной анимацией трёх точек и
  поддержкой reduced motion;
- текущая runtime-версия
  `8c344bba1c8e5f5e1a8cfaf9a0999c3187e850ab` опубликована только на
  `https://srtestrealme.ru:3001`; первая принятая версия —
  `146847aa11324565d2e0fd94c787578efa0ba25c`;
...
- подготовлен технический дизайн `AI-WIDGET-1`: плавающий виджет на страницах
  ...
  диалоговая сессия и CRM-симулятор;
- read-only проверка подтвердила, что preview отвечает `200` без общего HTTP
  access control; `noindex` не считается защитой;
- владелец принял открытый доступ для короткого preview-пилота на один–три дня;
  служебная ссылка, отдельная страница и дополнительный логин исключены;
...
- добавлен guard денежных сумм и исправлены ложные ценовые совпадения внутри
  слов «центр», «процентов» и неоднозначного «стоят»;
- ценовая регрессия прошла 17/17, boundary-регрессия — 11/11;
- FAQ переименован в `WIDGET_FAQ_V1_APPROVED.md` и утверждён только для
  закрытого пилота;
- зафиксирован QA-отчёт `WIDGET_CASCADE_V3_QA_20260727.md`;
