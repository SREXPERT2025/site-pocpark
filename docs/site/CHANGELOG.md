...
- подготовлен технический дизайн `AI-WIDGET-1`: закрытый маршрут на Mac
  Studio, restricted loopback gateway, feature flag/kill switch, 24-часовая
  HttpOnly pilot session и CRM-симулятор;
- read-only проверка подтвердила, что preview отвечает `200` без общего HTTP
  access control; `noindex` не считается защитой;
- дополнительная форма логина в AI-виджете не планируется: служебная ссылка
  обменивается на cookie и удаляется из URL;
