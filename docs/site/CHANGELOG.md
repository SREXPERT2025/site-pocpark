...
- отдельный registry на VPS, потому что интеграция существующей CRM пока не
  настроена;
- owner — директор;
- резервный ответственный указан как «Роб, Сергей» и требует уточнения;
- часовой пояс — Москва;
...
Рабочие часы, правило дублей, результаты закрытия и роли экспорта/удаления
остаются открытыми. Код, production, реальные сообщения и персональные данные
в этом блоке не изменялись.
