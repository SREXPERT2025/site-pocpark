...
## 2026-07-24 — read-only аудит `LEAD-OPS-002`
...
### Решение подготовлено, но не принято

Создан decision packet:
`docs/site/LEAD_OPS_002_DECISION_20260724.md`.

Рекомендуется использовать существующую CRM, если у неё есть рабочий API.
Если CRM нет — отдельный минимальный registry с transactional outbox вне Git
checkout. Код, production, реальные сообщения и персональные данные в этом
блоке не изменялись.
