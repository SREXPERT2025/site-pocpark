...
FAQ сохраняет статус `candidate` до повторного прогона `cascade v2` и ручной
проверки финальных ответов. Это QA-gate, а не отсутствие продуктовых решений.
...
- FAQ остаётся `candidate`: полный повторный прогон 230 сообщений и ручной QA
  ещё не выполнены.
...
Запустить полный набор из 230 сообщений через изолированный `cascade v3`
adapter, проверить отчёт и вручную просмотреть критические ответы. Запись в
грязный рабочий `POCPARK_AI` для этого не требуется. После QA-gate можно
утвердить FAQ и проектировать закрытый UI-стенд на Mac Studio.
