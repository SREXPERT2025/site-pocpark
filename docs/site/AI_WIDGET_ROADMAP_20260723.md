...
Синхронизировать принятый пакет `AI-WIDGET-0` из `docs/site/ai-widget/` в
соседний AI-проект и перезапустить `cascade v2` на актуальных фактах сайта.
После QA-gate можно проектировать закрытый UI-стенд на Mac Studio.
