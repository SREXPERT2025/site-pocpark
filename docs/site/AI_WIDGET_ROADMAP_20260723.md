...
Провести проверку `AI-WIDGET-1` директором на внешнем устройстве в течение
одного–трёх дней: desktop/mobile, тексты, скорость и ответы на реальные
вопросы без персональных данных. Затем собрать замечания, выполнить regression
и только после отдельного согласования подготовить накопленный пакет к выпуску
на VPS. MAX, CRM и production lead registry до следующего gate не подключать.
