...
Имя можно оставить необязательным.
...
Нужно решить, где будет доступен restricted AI gateway:
...
Сначала выполнить `AI-WIDGET-0`: утвердить FAQ, knowledge base, claim ledger,
data policy, владельца пилота и целевую архитектуру gateway. После этого
перезапустить `cascade v2` на актуальных фактах сайта и только затем проектировать
закрытый UI-стенд.
