...
проверены; в `ANALYTICS-001-C` созданы семь целей, этап продолжается
...
внутренней demo-страницы. Локальное исправление воспроизводит очередь
`dataLayer` после подключения Метрики и дедуплицирует повторную отправку.
Production build локально подтвердил ровно одну цель и при hard load, и при
SPA-переходе. Этот reliability patch ещё не опубликован.
...
- опубликовать и повторно проверить direct-load reliability patch;
- подтвердить поступление семи целей после обработки Метрикой;
