...
Дата последней сверки: 2026-07-28
Задача: `ANALYTICS-001`
Статус: `ANALYTICS-001-A` и `ANALYTICS-001-B` реализованы, опубликованы и
проверены; в `ANALYTICS-001-C` созданы восемь ручных целей, опубликованы
закрытая агрегированная аналитика обработки лидов и обезличенное событие
входа в demo/quiz; consent-gated интеграция GA4 опубликована в production
release `47529cc`, первое событие и Realtime ожидают подтверждения после
обработки Google
...
- первым основным сервисом выбрана Яндекс Метрика без GTM/GA4;
...
### 4.1. Production-расширение GA4 от 2026-07-28

По решению владельца сайта подготовлен второй прямой потребитель событий —
Google Analytics 4 без Google Tag Manager:

- в аккаунте `5423704` создан ресурс `РОСПАРК — основной сайт`;
- создан веб-поток `РОСПАРК — основной сайт` для
  `https://www.роспарк.рф`;
- идентификатор потока — `15338809658`, Measurement ID —
  `G-3Z9KNN3MMK`;
- `NEXT_PUBLIC_GOOGLE_ANALYTICS_ID=G-3Z9KNN3MMK` зафиксирован в
  `.env.example`;
- внешний `gtag.js` загружается только на production-host, только после
  сохранённого `accepted` и никогда не загружается в `/admin`;
- рекламное хранилище, Google Signals, рекламные user data и персонализация
  рекламы выключены;
- начальный и последующие SPA pageview передаются без query string;
- существующие `rospark_*`-события повторно используются через тот же
  privacy-safe `dataLayer`;
- отказ или отзыв согласия сохраняет fail-closed поведение;
- Политика обработки персональных данных дополнена описанием GA4 и способом
  отзыва согласия;
- пакет опубликован на VPS в release
  `47529cc83489534c4ba25cfe7a069e590d924851`;
- production environment и build содержат
  `NEXT_PUBLIC_GOOGLE_ANALYTICS_ID=G-3Z9KNN3MMK`;
- production browser QA подтвердил внешний `gtag.js` после сохранённого
  analytics consent и отсутствие GA4 в `/admin`;
- Google Analytics сразу после выпуска ещё показывает «данные не получены»;
  первое событие и Realtime проверяются после периода обработки до 48 часов.

Автоматические pageview при изменении browser history отключены в настройках
улучшенной статистики потока. Загрузка обычной страницы и остальные функции
улучшенной статистики сохранены. Сайт отправляет начальный и последующие SPA
pageview вручную, поэтому дубли между кодом сайта и history-based событиями GA4
исключены на уровне настройки потока.
...
2026-07-25 опубликована следующая часть client-side воронки:
...
- `YandexMetrika` воспроизводит событие через существующий
  `dataLayer → reachGoal` контракт;
- production release:
  `89c045d79535169527347c40c438971fb560995d`;
- в счётчике создана восьмая ручная цель
  `Воронка — вход в demo/quiz`, ID `588884963`;
- выполнено по одному контролируемому production-переходу с
  `/resheniya/biznes-centry` в `/demo` и `/quiz?source=request` без отправки
  форм и PII;
- после обработки отчёт цели зарегистрировал один целевой визит и четыре
  просмотра в нём; оба перехода выполнялись внутри одного визита, поэтому
  доставка цели подтверждена, а отдельная сверка параметров `destination` /
  `landing_group` остаётся пунктом последующей проверки.
...
- подтверждённое отчётом Метрики production-накопление
  `rospark_funnel_entry` по категориям landing page;
...
- создать или выбрать GA4 property/web stream и предоставить Measurement ID;
- перед production-выпуском проверить DebugView/Realtime без PII и
  query string;
- добавить GA4 Measurement ID в production environment до сборки;
- выпустить GA4 только в составе отдельного согласованного накопительного
  релиза и повторить consent smoke для обоих сервисов.
...
- после обработки данных подтвердить два контрольных достижения
  `rospark_funnel_entry` и разрешённые значения `destination` /
  `landing_group`;
