...
После закрытия `LEAD-OPS-002` локально добавлена серверная часть воронки в
закрытый `/admin/leads`:
...
- выпустить подготовленные агрегированные статусы обработки отдельным
  production release;
