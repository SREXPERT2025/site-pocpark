...
Статус: `ANALYTICS-001-A` и локальная часть `ANALYTICS-001-B` реализованы и
...
## 4. Локальная интеграция `ANALYTICS-001-B`
...
- добавить `NEXT_PUBLIC_YANDEX_METRIKA_ID=110980303` в production environment
  до сборки;
...
- создать цели в интерфейсе Метрики на этапе `ANALYTICS-001-C`.

## 5. Что пока нельзя считать

До production-выпуска loader и появления серверной связи пока нельзя
достоверно получить:
...
Локальный `dataLayer` сам по себе остаётся интерфейсом интеграции. Рабочий сбор
Метрики начнётся только после отдельного production-выпуска loader.
...
способ изменения выбора реализованы локально.
...
- production environment, CSP/headers, release и внешний smoke.
