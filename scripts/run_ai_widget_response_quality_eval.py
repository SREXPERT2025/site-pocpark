#!/usr/bin/env python3
"""Локальная ручная регрессия шести вопросов с двух лендингов."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=int,
        help="Номера кейсов 1-6; без аргументов выполняются все.",
    )
...
def main() -> int:
    ...
    if not selected_cases or min(selected_cases) < 1 or max(selected_cases) > len(CASES):
        raise SystemExit("Номера кейсов должны быть в диапазоне 1-6.")
    engine = PilotEngine(
